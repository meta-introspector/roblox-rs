...
/// Generate actor system code
pub fn generate_actor_system() -> String {
    r#"-- Actor System for Roblox-RS
...
function RobloxRS.Actors.spawn(handler, initialState, name)
    ...
    RobloxRS.Actors.actors[id] = actor
    RobloxRS.Actors.mailboxes[id] = actor.mailbox
    ...
    script.Source = [[
        ...
        -- Get registered handler and state from RobloxRS
        local function processNext()
            ...
            if success and message then
                ...
                local handlerResult = actor:SendMessage("ProcessMessage", actorId, message)
    ...
    robloxActor:BindToMessage("GetNextMessage", function(requestId)
        ...
        local actor = RobloxRS.Actors.actors[id]
    ...
    robloxActor:BindToMessage("ProcessMessage", function(requestId, message)
        ...
        local success, result = pcall(function()
            return handler(actor.state, message)
        end)
        ...
        if message and message._responseChannel then
            RobloxRS.Actors.callbacks[message._responseChannel](success, result)
    ...
    -- Add actor to workspace (or another container)
    robloxActor.Parent = workspace
...
function RobloxRS.Actors.send(actorId, message, priority)
    local actor = RobloxRS.Actors.actors[actorId]
    if not actor then
...
function RobloxRS.Actors.ask(actorId, message, timeout)
    local actor = RobloxRS.Actors.actors[actorId]
    if not actor then
        return false, "Actor not found"
    end
    ...
    RobloxRS.Actors.callbacks[responseChannel] = function(success, result)
        ...
        -- Clean up
        RobloxRS.Actors.callbacks[responseChannel] = nil
    end
    ...
    -- Send the message
    RobloxRS.Actors.send(actorId, messageWithResponse, MessagePriority.HIGH)
    
    -- Set up timeout if specified
    if timeout then
        task.delay(timeout, function()
            if not responsePromise._resolved then
                RobloxRS.Actors.callbacks[responseChannel](false, "Timeout waiting for response")
            end
        end)
    end
    
    -- Return promise-like object with then method
    function responsePromise:then(callback)
...
function RobloxRS.Actors.isActorAlive(actorId)
    return RobloxRS.Actors.actors[actorId] ~= nil
end

-- Terminate an actor
function RobloxRS.Actors.terminate(actorId)
    local actor = RobloxRS.Actors.actors[actorId]
    if not actor then
        return false, "Actor not found"
    end
    ...
    -- Record termination
    RobloxRS.Actors.terminated[actorId] = os.clock()
    ...
    RobloxRS.Actors.actors[actorId] = nil
    RobloxRS.Actors.mailboxes[actorId] = nil
    
    -- Terminate children if any
    for childId, _ in pairs(actor.children) do
        RobloxRS.Actors.terminate(childId)
    end
...
function RobloxRS.Actors.monitorActor(actorId, callback)
    local actor = RobloxRS.Actors.actors[actorId]
    if not actor then
...
function RobloxRS.Actors.supervise(childHandler, initialState, name, options)
    ...
    local supervisor = RobloxRS.Actors.spawn(function(state, message)
        if message.type == "monitor" then
            ...
            if restartCount <= state.maxRestarts then
                ...
                task.delay(state.restartDelay, function()
                    local newChild = RobloxRS.Actors.spawn(state.childHandler, state.initialState, state.childName)
                    
                    -- Update current child
                    state.currentChild = newChild.id
                    
                    -- Monitor the new child
                    RobloxRS.Actors.monitorActor(newChild.id, function(actorId)
                        RobloxRS.Actors.send(state.id, {
                            type = "monitor",
                            actorId = actorId
                        })
                    end)
                    ...
                    task.delay(state.resetCountAfter, function()
                        if RobloxRS.Actors.isActorAlive(newChild.id) then
        ...
        elseif message.type == "forward" then
            ...
            if state.currentChild then
                return RobloxRS.Actors.ask(state.currentChild, message.content)
            else
    ...
    local supervisorState = RobloxRS.Actors.actors[supervisor.id].state
    supervisorState.id = supervisor.id
    
    -- Create initial child
    local child = RobloxRS.Actors.spawn(childHandler, initialState, name)
    supervisorState.currentChild = child.id
    supervisorState.children[child.id] = true
    
    -- Monitor child
    RobloxRS.Actors.monitorActor(child.id, function(actorId)
        RobloxRS.Actors.send(supervisor.id, {
            type = "monitor",
            actorId = actorId
        })
    end)
    ...
    local handle = {
        ...
        send = function(self, message)
            return RobloxRS.Actors.send(supervisor.id, {
                type = "forward",
                content = message
            })
        end,
        ask = function(self, message, timeout)
            return RobloxRS.Actors.ask(supervisor.id, {
                type = "forward",
                content = message
            }, timeout)
        end,
...
function RobloxRS.Actors.createPool(workerCount, workerHandler, initialState, options)
    local opts = options or {
        distributionStrategy = "round-robin"  -- or "least-busy"
    }
    
    -- Create a pool manager actor
    local poolManager = RobloxRS.Actors.spawn(function(state, message)
        ...
        elseif message.type == "work" then
            ...
            if selectedWorker then
                ...
                -- Forward the work
                local response = RobloxRS.Actors.ask(selectedWorker.id, message.payload)
    ...
    -- Create workers
    local poolState = RobloxRS.Actors.actors[poolManager.id].state
    for i = 1, workerCount do
        local worker = RobloxRS.Actors.spawn(workerHandler, initialState, "Worker_" .. i)
        ...
        RobloxRS.Actors.monitorActor(worker.id, function(actorId)
            -- Worker terminated, replace it
            for i, w in ipairs(poolState.workers) do
                if w.id == actorId then
                    local newWorker = RobloxRS.Actors.spawn(workerHandler, initialState, "Worker_" .. i)
                    ...
                    RobloxRS.Actors.monitorActor(newWorker.id, function(actorId)
                        -- This will recursively replace terminated workers
                        RobloxRS.Actors.send(poolManager.id, {
    ...
    local poolHandle = {
        ...
        submit = function(self, payload, timeout)
            return RobloxRS.Actors.ask(self.id, {
                type = "work",
                payload = payload
            }, timeout)
        end,
        ...
        terminate = function(self)
            ...
            local workers = RobloxRS.Actors.actors[self.id].state.workers
            
            -- Terminate all workers first
            for _, worker in ipairs(workers) do
                RobloxRS.Actors.terminate(worker.id)
            end
...
function RobloxRS.Actors.createSharedState(initialState)
    -- Create an actor to manage the shared state
    local stateManager = RobloxRS.Actors.spawn(function(state, message)
        ...
        elseif message.type == "set" then
            ...
            if state.subscribers[message.key] then
                for _, subInfo in ipairs(state.subscribers[message.key]) do
                    RobloxRS.Actors.send(subInfo.actorId, {
                        type = "state_changed",
                        key = message.key,
                        value = message.value,
                        id = subInfo.id
...
RobloxRS.Actors.Priority = {
    HIGH = MessagePriority.HIGH,
    NORMAL = MessagePriority.NORMAL,
    LOW = MessagePriority.LOW
}

return RobloxRS.Actors"#.to_string()
}

// Test the actor system in a simulated Roblox environment
pub fn test_actor_system() {
    // Generate the actor system code
    let actor_code = generate_actor_system();
    ...
    // Write the actor system to a file
    let actor_file = output_dir.join("actors.lua");
    fs::write(&actor_file, &actor_code).expect("Failed to write actor file");
    
    println!("Actor system code generated:");
    println!("- File: {}", actor_file.display());
    println!("- Size: {} bytes", actor_code.len());
    
    // Create a test script that uses the actor system
    let test_script = r#"-- Actor system test
local Actors = require("actors")

-- Create an actor that counts
...
local response = counter:ask({type = "increment", value = 5})
response:then(function(success, result)
    print("Counter incremented to: " .. tostring(result))
end)

-- Create a supervised actor
local fragile = Actors.supervise(
    function(state, message)
        if message.type == "crash" then
            error("Actor crashed intentionally")
        elseif message.type == "work" then
            state.workDone = (state.workDone or 0) + 1
...
local pool = Actors.createPool(4, function(state, message)
    ...
    local result = fib(message.n or 10)
    return {
        input = message.n,
        result = result
    }
...
-- Submit work to the pool
for i = 1, 5 do
    pool:submit({n = i + 20}):then(function(success, result)
        if success then
            print("Fibonacci result: " .. tostring(result.result))
        else
...
-- Create actors that use the shared state
local stateUsers = {}
for i = 1, 3 do
    stateUsers[i] = Actors.spawn(function(state, message)
        if message.type == "increment_users" then
            -- Get current value
            local users = shared:get("users")
            -- Update with new value
            shared:set("users", users + 1)
            return shared:get("users")
        elseif message.type == "state_changed" then
            print("Actor " .. state.id .. " received state change: " .. 
                  message.key .. " = " .. tostring(message.value))
            return "Acknowledged"
        end
    end, {id = "StateUser_" .. i})
    
    -- Subscribe to changes
    shared:subscribe("users", stateUsers[i].id)
end

print("Tests started successfully")
"#;
    
    let test_file = output_dir.join("actor_test.lua");
    fs::write(&test_file, test_script).expect("Failed to write test file");
    
    println!("\nTest script generated: {}", test_file.display());
    println!("\nActor system ready for use in Roblox!");
}

// Main function to run the tests
fn main() {
    println!("\n===== RobloxRS Actor System Test =====\n");
